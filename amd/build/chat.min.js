        ...
        init: function(courseid, release) {
            ...
            function sendMessage() {
                ...
                if (messagesend.length > 1 || chat.mediaRecordUrl) {
                    ...
                    // Detectar si es una solicitud de creación de curso
            ...
            // Función para detectar solicitudes de creación de curso
                ...
                for (var i = 0; i < patterns.length; i++) {
                    ...
                    if (match) {
                        return {
                            ...
                            topic: match[1].trim(),
                            weeks: match[2] ? parseInt(match[2]) : 8
                        };
                    }
                }
                
                return { isCourseRequest: false };
            }
            
            // Función para manejar la creación de cursos
            function handleCourseCreation(detection, fullMessage) {
                // Agregar mensaje del usuario
                geniaiareamensagens.append('<div class="geniai-message geniai-user format-text">' + fullMessage + '</div>');
                
                // Agregar mensaje de carga
                var loadingId = "loading-" + Math.random().toString(16).slice(2);
                var loadingMessage = '<div id="' + loadingId + '" class="geniai-message geniai-server format-text">' +
                    '<div style="padding: 15px; background: #fff3cd; border-left: 4px solid #ffc107; border-radius: 4px;">' +
                    '⏳ Creando curso sobre "<strong>' + detection.topic + '</strong>" con ' + detection.weeks + ' semanas...<br>' +
                    '<small>Esto puede tomar 1-2 minutos, por favor espera.</small>' +
                    '</div></div>';
                geniaiareamensagens.append(loadingMessage);
                geniaiscrollarea.scrollTop = geniaiscrollarea.scrollHeight;
                ...
                ajax.call([{
                    ...
                    var successMessage = '<div class="geniai-message geniai-server format-text">' +
                        '<div style="padding: 15px; background: #d4edda; border-left: 4px solid #28a745; border-radius: 4px;">' +
                        '<strong>✅ ¡Curso creado exitosamente!</strong><br><br>' +
                        '<strong>' + result.coursename + '</strong><br>' +
                        '<small>' + detection.weeks + ' semanas</small><br><br>' +
                        '<a href="' + result.courseurl + '" target="_blank" style="color: white; background: #28a745; padding: 8px 16px; text-decoration: none; border-radius: 4px; display: inline-block; margin-top: 10px;">' +
                        '📚 Ver curso →' +
                        '</a></div></div>';
                    geniaiareamensagens.append(successMessage);
                    geniaiscrollarea.scrollTop = geniaiscrollarea.scrollHeight;
                    ...
                    // Agregar mensaje de error con detalles completos
                    var errorMessage = "Error desconocido al crear el curso";
                    var errorDetails = "";
                    
                    if (error.exception) {
                        errorMessage = error.exception.message || error.exception;
                        errorDetails = error.exception.debuginfo || "";
                    ...
                    var errorHtml = '<div class="geniai-message geniai-server format-text">' +
                        '<div style="padding: 15px; background: #f8d7da; border-left: 4px solid #dc3545; border-radius: 4px;">' +
                        '<strong>❌ Error al crear el curso</strong><br>' +
                        '<small>' + errorMessage + '</small>';
                    
                    if (errorDetails) {
                        errorHtml += '<br><br><details><summary>Ver detalles técnicos</summary><pre style="font-size: 11px; max-height: 200px; overflow: auto;">' + 
                            errorDetails + '</pre></details>';
                    }
                    
                    errorHtml += '</div></div>';
                    geniaiareamensagens.append(errorHtml);
                    geniaiscrollarea.scrollTop = geniaiscrollarea.scrollHeight;
                });
            }
        },
